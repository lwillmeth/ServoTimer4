...
{ 
  ...
  {
	TCNT4 =  servos[Channel].remainder;   // yes, set count for overflow after remainder ticks
  }  
  else if(ISRCount >  servos[Channel].counter)  
  {
	...
	TCNT4 = 0;    // reset the clock counter register
...
uint8_t ServoTimer4::attach(int pin)
{
	if( isStarted == false)
	 initISR();    
	if(this->chanIndex > 0)  
...
{   
	...
	TCNT4 = 0;     // clear the timer2 count 
	TIFR4 = _BV(TOV4);  // clear pending interrupts; 
	TIMSK4 =  _BV(TOIE4); // enable the overflow interrupt	  
